...
def submit(record):
    ...
    with _queue_lock:
        # Read once and reuse: the queue is parsed line by line, and this
        # runs on every public submission while holding the lock.
        pending = read_all(status=PENDING)
        if len(pending) >= config.MAX_PENDING_SUBMISSIONS:
