...
- 📊 **Rich Agent Database** - Curated collection of 308 AI agents, frameworks and developer tools
...
Every setting is an environment variable, read once in `backend/config.py`, and
the app runs with an empty `.env`. See **[docs/CONFIG.md](docs/CONFIG.md)** for
the full table, or `ai-agent-discovery/.env.example` for the same list with
defaults.
...
| `/tech` | Every technology, filterable |
